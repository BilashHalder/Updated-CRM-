...
export default function SalaryEdit(props) {
...
const [hra, setHra] = useState(props.data.hra);
const [conveyance, setConveyance] = useState(props.data.conveyance);
const [medical, setMedical] = useState(props.data.medical);
const [special, setSpecial] = useState(props.data.special);
const [pf, setPf] = useState(props.data.pf);
...
//Common Functions For All
 ...
 const formHandler=(e)=>{
  ...
  if(!basic || !hra || !conveyance || !medical || !special || !pf || !insurance || !tax){
   ...
   instance.put(`salary/${props.data.id}`,data)
      .then((response)=> {
        setAlertShow(true);
        setMessage("Salary Information Updated");
        console.log(props)
 ...
 const resetForm=()=>{
  ...
  return (
      ...
      <Typography align={'center'} variant={'h5'} sx={{'marginBottom':'5%'}}>Edit Salary Information</Typography>
