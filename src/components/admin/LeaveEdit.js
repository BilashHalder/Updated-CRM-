...
export default function LeaveEdit(props) {
   const{fun,data}= props
...
const [casual, setcasual] = useState(data.casual);
const [sick, setsick] = useState(data.sick);
const [maternity, setmaternity] = useState(data.maternity);
const [bereavement, setbereavement] = useState(data.bereavement);
...
//Common Functions For All
 ...
 const formHandler=(e)=>{
  ...
  instance.put(`leave/${id}`,data)
      .then((response)=> {
        setAlertShow(true);
        setMessage("Leave Information Updated");
 ...
 const resetForm=()=>{
  ...
  return (
      ...
      <Typography align={'center'} variant={'h5'} sx={{'marginBottom':'5%'}}>Edit Leave Information</Typography>
