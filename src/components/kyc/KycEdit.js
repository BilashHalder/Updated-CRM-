...
export default function KycEdit(props) {
    const { data}=props;
    // ``, ``, ``, ``, ``, `user_id`, `user_type`
  ...
  //Form Data
  const [pan, setPan] = useState(data.adhar_no);
  const [adhar, setAdhar] = useState(data.pan_no);
  const [address, setAddress] = useState(data.address)
  const [adhar_verified, setadhar_verified] = useState(data.adhar_verified)
  const [pan_verified, setpan_verified] = useState(data.pan_verified)
